...
namespace Day10
{
    internal class AocDay10
    {
        ...
        private static int Part1(List<int> joltages)
        {
            // outlet = 0
            joltages.Add(0);
            var allJoltages = joltages.OrderBy(j => j).ToList();

            var threes = 1; // device and largest
            var ones = 0;

            for (var i = 1; i < allJoltages.Count; i++)
            {
                var delta = allJoltages[i] - allJoltages[i - 1];
                if (delta == 1) ones++;
                if (delta == 3) threes++;
        ...
        // etcetera.
        // formula for (n>= 3) : permutations = 2^(n-1) - 2^(n-3) + 1;
        ...
        private static long Part2(List<int> joltages)
        {
            joltages.Add(0);
            var allJoltages = joltages.OrderBy(j => j).ToList();
            allJoltages.Add(allJoltages[^1]+3);
            ...
            for (var i = 1; i < allJoltages.Count; i++)
            {
                ...
                else
                {
                    if (groupCount > 1)
                    {
                        permutations *= groupCount switch
                        {
                            2 => 2,
                            _ => (int) (Math.Pow(2, groupCount - 1) - Math.Pow(2, groupCount - 3) + 1)
                        };
                    }
                    groupCount = 0;
                }
